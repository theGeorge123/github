...
function Controller.Init(remotes,parent,buttonFactory,labelFactory,colors)
 ...
 local function starter(labelText,textValue)local b=buttonFactory(assist,labelText,colors.panel);b.Size=UDim2.new(.32,-4,1,0);b.TextSize=11;b.Activated:Connect(function()if not turnToken or pending then return end;local prefix=box.Text==""and""or(box.Text:sub(-1)==" "and""or" ");box.Text=box.Text..prefix..textValue;box.CursorPosition=#box.Text+1;box:CaptureFocus()end)end
 ...
 local function flash(text)flashUntil=os.clock()+2;send.Text=text end;send.Activated:Connect(function()if not session or not turnToken then flash(session and"PRACTICE OVER • PRESS CLOSE"or"NO ACTIVE TURN");return end;if pending then flash("SENDING…");return end;if box.Text:match("^%s*$")then flash("TYPE YOUR ARGUMENT FIRST");box:CaptureFocus();return end;flashUntil=0;turnDeadline=nil;send.Text="SENDING…";submission+=1;pending=submission;submit:FireServer({Action="SubmitArgument",SessionId=session,RoundGeneration=round,TurnToken=turnToken,SubmissionId=pending,Text=box.Text})end)
